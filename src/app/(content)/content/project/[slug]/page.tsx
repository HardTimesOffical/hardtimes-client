...
export default function ProjectPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-background"><div className="w-6 h-6 border-2 border-orange-500 border-t-transparent rounded-full animate-spin" /></div>;
  if (!project) return <div className="p-10 text-center font-bold text-muted-foreground uppercase text-[10px]">Проект не найден</div>;
                ...
                {project.owner ? (
                  ...
                  >
                    <div className="w-10 h-10 rounded-full bg-secondary border border-border overflow-hidden flex items-center justify-center shrink-0 group-hover:border-orange-500 transition-all shadow-inner">
                      {project.owner.avatar ? (
                        <img 
                          ...
                          referrerPolicy="no-referrer" // Разрешает загрузку с Pinterest/внешних сайтов
                          onError={(e) => {
                            // Если картинка всё же не загрузилась
                            e.currentTarget.style.display = 'none';
                            const parent = e.currentTarget.parentElement;
                            if (parent) {
                              parent.innerHTML = `<span class="text-xs font-black text-orange-500">${project.owner.username?.charAt(0)}</span>`;
                            }
                          }}
                        />
                      ) : (
                    ...
                    <div className="flex flex-col min-w-0">
                      <span className="text-xs font-black uppercase truncate tracking-tight group-hover:text-orange-500 transition-colors">
